...
def Articulation(img_gray,articulation_operator):
    ...
    if articulation_operator=='Canny':
        ...
        return np.average(cv2.Canny(img_gray,0,1000).ravel())

    if articulation_operator=='Sobel-x':
        
        return np.average(cv2.Sobel(img_gray, cv2.CV_64F, 1, 0, ksize=5).ravel())
    
    if articulation_operator=='Sobel-y':
    
        return np.average(cv2.Sobel(img_gray, cv2.CV_64F, 0, 1, ksize=5).ravel())
    
    if articulation_operator=='Laplacian':
        
        return np.average(cv2.Laplacian(img_gray, cv2.CV_64F).ravel())